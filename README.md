...
### v1.3.0 (2026-07)
- ✨ **强类型模型重构**：全面接入 Freezed 实体模型，取代脆弱的动态字典（Map）。
- ✨ **图片永久本地缓存**：新增 `ImageCacheManager`，网络图片静默下载至本地，无网环境毫秒级闪开。
- ✨ **真正的离线队列**：新增 `SyncQueueManager` 同步队列，断网操作自动挂起，网络恢复后无缝上传。
- ✨ **Provider 状态管理**：重构首页和日记列表，摒弃陈旧的 `setState`，实现数据变动瞬间响应。
- ✨ **动态深色模式**：新增 `getDarkTheme()` 支持，清理硬编码颜色，完美适配系统日夜交替。
- ✨ **性能分页加载**：日记列表引入滚动触底拉取（limit/offset），首屏提速。
- ✨ **自定义图片压缩**：新增「上传原图」开关，平衡画质与云盘存储空间。
