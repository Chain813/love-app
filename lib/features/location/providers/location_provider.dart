...
class LocationProvider extends ChangeNotifier {
  ...
  List<Map<String, dynamic>> _partnerHistory = [];
  ...
  List<Map<String, dynamic>> get myHistory => _myHistory;
  List<Map<String, dynamic>> get partnerHistory => _partnerHistory;
  ...
  /// 加载历史轨迹
  Future<void> loadLocationHistory({required String myId, required String partnerId}) async {
    try {
      final history = await LeanCloudService.fetchLocationHistory();
      _myHistory = history.where((item) => item['userId'] == myId).toList();
      _partnerHistory = history.where((item) => item['userId'] == partnerId).toList();
      notifyListeners();
    } catch (e) {
      debugPrint('加载历史轨迹失败: $e');
    }
  }

  /// 刷新双方位置
  Future<void> refreshLocations({required String myId, required String partnerId}) async {
    _isLoading = true;
    notifyListeners();
    ...
    await Future.wait([
      ...
      loadLocationHistory(myId: myId, partnerId: partnerId),
    ]);
